...
// ── Pins ─────────────────────────────────────────────────────────────────────
// The 4.2.2 boards show up with two different LCD wirings in the wild.
// A "blue screen" after boot almost always means the LCD wiring variant and
// the chosen U8g2 SPI backend don't match.
//
// Since the current build still blue-screens on your machine, default to the
// alternate VET6 / SPI1 mapping here. If that is wrong for a different board,
// change LCD_VARIANT to LCD_VARIANT_RET6 and rebuild.
#define LCD_VARIANT_RET6 1
#define LCD_VARIANT_VET6 2
#define LCD_VARIANT      LCD_VARIANT_VET6

#if LCD_VARIANT == LCD_VARIANT_RET6
  #define LCD_CLK  PB13   // EXP3_06  ST7920 CLK  -> SPI2_SCK
  #define LCD_DATA PB15   // EXP3_08  ST7920 DATA -> SPI2_MOSI
...
#elif LCD_VARIANT == LCD_VARIANT_VET6
  #define LCD_CLK  PA5    // ST7920 CLK  -> SPI1_SCK
  #define LCD_DATA PA7    // ST7920 DATA -> SPI1_MOSI
  #define LCD_CS   PA4    // ST7920 CS
  #define ENC_A    PB10
  #define ENC_B    PA6
  #define ENC_BTN  PC5
  #define BUZZ     -1     // no beeper on this variant
#else
  #error "Unsupported LCD_VARIANT"
#endif
...
//
// Use the hardware SPI instance that matches the actual LCD wiring:
//   RET6 boards wire the display to SPI2 (PB13/PB15)
//   VET6 boards wire the display to SPI1 (PA5/PA7)
// Then cap the bus clock to 1MHz, comfortably below the ST7920 limit.
// ────────────────────────────────────────────────────────────────────────────
#if LCD_VARIANT == LCD_VARIANT_RET6
U8G2_ST7920_128X64_F_2ND_HW_SPI lcd(U8G2_R0, LCD_CS, U8X8_PIN_NONE);
#else
U8G2_ST7920_128X64_F_HW_SPI lcd(U8G2_R0, LCD_CS, U8X8_PIN_NONE);
#endif
...
// Forward declarations normally autogenerated for .ino sketches.
...
void beep(uint16_t f=2000, uint16_t ms=20) {
#if BUZZ >= 0
  tone(BUZZ, f, ms);
#else
  (void)f;
...
void setup() {
  ...
  // ── LCD FIRST — always, before SD, before anything else ────────────────
  // If SD init runs first and hangs (wrong pins, no card), lcd.begin() is
  // never called and you get a permanent blue/blank screen.
  delay(150);                 // let display power stabilise after boot
  lcd.setBusClock(1000000);   // 1MHz — safe for ST7920 (max 2.5MHz)
  lcd.begin();
  delay(30);

  // Splash screen — confirms LCD is working
  lcd.clearBuffer();
  lcd.setFont(u8g2_font_6x12_tr);
  lcd.drawStr(20, 24, "Pen Plotter");
  lcd.setFont(u8g2_font_5x8_tr);
  lcd.drawStr(22, 38, "Jacob Majors");
  lcd.drawStr(28, 50, "Troy Pappas");
  lcd.sendBuffer();

  beep(1500,80); delay(100); beep(2200,80);
  delay(800);   // show splash briefly

  // SD init — AFTER LCD so display always works regardless of SD state
  sdOk = initSD();
  sdChecked = true;
  if (sdOk) scanFiles();
  redraw = true;
